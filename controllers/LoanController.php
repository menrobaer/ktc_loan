  ...
  public function actionCreate()
  {
    ...
    if ($model->load(Yii::$app->request->post())) {
      ...
      try {
        ...
        $bulkData = [];
        $paymentDay = new DateTime($model->payment_day);
        $interestRateAmount = $model->interest_rate_amount / $model->payment_term_id;
        for ($i = 1; $i <= $model->payment_term_id; $i++) :
          $originalBalanceInvertal -= $model->original_installment_amount;

          $paymentDate = $paymentDay->format('Y-m-d');
          $paymentDay->modify('+1 month');
          $amount = $i == 1 ? floatval($model->installment_amount + $model->service_charge_amount) : $model->installment_amount;
          $originalBalance = $i == $model->payment_term_id ? 0 : floatval($originalBalanceInvertal);

          $bulkData[] = [
            $model->id,
            $paymentDate,
            floatval($amount),
            floatval($model->original_installment_amount),
            floatval($interestRateAmount),
            floatval($originalBalance),
            $i == 1 ? floatval($model->service_charge_amount) : 0,
            0
          ];
        endfor;
        if (!empty($bulkData)) {
