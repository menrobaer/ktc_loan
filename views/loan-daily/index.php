...
$currency = Yii::$app->setup->currency();
      ...
      <?= GridView::widget([
        ...
        'columns' => [
          ...
          [
            ...
            'value' => function ($model) use ($utils) {
              return $utils->DollarFormat($model->original_amount);
            }
          ],
          [
          ...
          [
            ...
            'buttons' => [
              ...
              'void' => function ($url, $model) {
                if ($model->status != Loan::CANCELLED) {
                  return Html::a('<i class="fas fa-times"></i>', '#', [
                    ...
                    'data' => [
                      'url' => Url::toRoute(['loan-daily/void', 'id' => $model->id]),
...
$this->registerJs("
    ...
    $('#table-loan td').click(function (e) {
        ...
        if(e.target == this)
            location.href = '" . Url::to(['loan-daily/view']) . "?code=' + code;
    });
